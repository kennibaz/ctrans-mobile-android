        ...
        querySnapshot.docChanges().forEach((change) => {
          if (change.type === 'modified') {
            ...
            if (
              ...
              uploadingOrder = rawData;
              //files removal from device
              const ordersToDelete =
                uploadingOrder.pickup.pickup_conditions.upload.images_to_upload;
              ordersToDelete.forEach((order) => {
                RNFS.unlink(order).then(() => {
                  console.log('File DELETED');
                });
              });

              // call for a bOL generation
              const result = async () => {
                db.doc(
                  `carriers-records/c87U6WtSNRybGF0WrAXb/orders/${rawId}`,
                ).update({
                  ...
                  'pickup.pickup_conditions.upload.images_to_upload': [],
                });
              };
              result();
            }
            ...
            if (
              ...
              const result = async () => {
                db.doc(
                  ...
                  'delivery.delivery_conditions.upload.images_to_upload': [],
                });
              };
              result();
            }
